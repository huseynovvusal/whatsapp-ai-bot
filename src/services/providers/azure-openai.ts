...
export class AzureOpenAILLMProvider implements LLMProvider {
  ...
  constructor(apiKey: string, endpoint: string, deployment: string, apiVersion: string = "2023-05-15") {
    this.openai = new AzureOpenAI({ apiKey, endpoint, apiVersion })
  ...
  async askLLM(
    ...
    const res = await this.openai.chat.completions.create({
      model: this.deployment, // For Azure, the deployment name goes in the model field
      messages: [
...
export class AzureOpenAIEmbeddingProvider implements EmbeddingProvider {
  ...
  async embedBatch(texts: string[]): Promise<number[][]> {
    ...
    const res = await this.openai.embeddings.create({ model: this.deployment, input: texts })
